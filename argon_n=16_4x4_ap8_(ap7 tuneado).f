      ...
      open(11,file='radios1.txt')
      open(12,file='radios2.txt')
      open(13,file='radios3.txt')
      open(14,file='radios4.txt')
      open(15,file='radios5.txt')
      open(16,file='radios6.txt')
      open(17,file='radios7.txt')
      open(18,file='radios8.txt')
      open(19,file='radios9.txt')
      open(20,file='radios10.txt')
...
!!!!!!!Calculo aceleraci¢n siguiente
      ...
      !Aceleraci¢n total de cada part¡cula
       ...
       enddo
        ...
        v=omega+dt*0.5d0*a1
           !Reescalo la velocidad en tiempos indicados
        do j=1,10
          if((t.le.dfloat(j)*60.d0).and.(t+dt.ge.dfloat(j)*60.d0)) then
           v=v*1.1d0
          endif
        enddo
        ...
        write(5,*) t, Ec, Ep, Et
         ...
         !Temperatura
         do j=1,10
           if((t.ge.dfloat(j-1)*60.d0).and.(t.lt.dfloat(j)*60.d0)) then
             cont(j)=cont(j)+1
             do i=1,n
             Temp(j)=Temp(j)+0.5d0*(v(i,1)**2.d0+v(i,2)**2.d0)/dfloat(n)
             enddo
             fluc(j)=fluc(j)+(r(6,1)-r(7,1))**2.d0+(r(6,2)-r(7,2))**2.d0
           endif
         enddo

         do j=1,10
           if ( (mod(p,10).eq.0).and.(t.ge.dfloat(j-1)*60.d0).and.
     &(t.lt.dfloat(j)*60.d0) )then
             entero_aux=10+j
             do i=1,5
               dist(1)=min( abs(r(i,1)-r(6,1)), 4.d0-abs(r(i,1)-r(6,1)))
               dist(2)=min( abs(r(i,2)-r(6,2)), 4.d0-abs(r(i,2)-r(6,2)))
               write(entero_aux,*) sqrt(dist(1)**2.d0+dist(2)**2.d0)
             enddo
             do i=7,n
               dist(1)=min( abs(r(i,1)-r(6,1)), 4.d0-abs(r(i,1)-r(6,1)))
               dist(2)=min( abs(r(i,2)-r(6,2)), 4.d0-abs(r(i,2)-r(6,2)))
...
!!!!!!FIN DE ALGORTIMO
      ...
      do i=1,10
        Temp(i)=Temp(i)/dfloat(cont(i))
        fluc(i)=fluc(i)/dfloat(cont(i))

        write(9,*)  Temp(i), fluc(i)
      enddo
